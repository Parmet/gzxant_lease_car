...
public class UniqueNoUtils {
	...
	public static synchronized String next(UniqueNoType type) {
		...
		return type+String.format("%1$tY%1$tm%1$td%1$tk%1$tM%1$tS%2$03d%3$03d", cal, ipMix, seq++).substring(type.toString().length());
	}

	public static synchronized String next(String pretex) {
		if (seq > ROTATION){
			seq = 0;
		}
		if (ipMix == 0) {
		...
		cal.setTimeInMillis(System.currentTimeMillis());
		return pretex+String.format("%1$tY%1$tm%1$td%1$tk%1$tM%1$tS%2$03d%3$03d", cal, ipMix, seq++).substring(2);
	}
